...
App / App Manager 는 각자 Server와 UDS(Unix Domain Socket)로 연결됩니다.
...
RPC를 위해 호출을 패킷으로 직렬화해 보내고, Server가 그것을 다시 역직렬화 합니다.
...
이떄, Server가 그림을 화면에 내보내는 과정은 2단계 + SPI 전송용 스레드로 동작합니다.
