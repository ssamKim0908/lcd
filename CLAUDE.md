...
세 종류의 프로세스로 구성. **Server가 시스템의 init 역할** — 부팅 시 server가 먼저 뜨고 launcher를 fork+exec 한다. Game/MP3 는 launcher 가 fork 한다.
...
│   - IpcServer        (accept loop, focus stack)        │
│   - RenderServerStub (per session: recv → 역직렬화 →   │
│                       IRenderCommand → execute)        │
│   - InputDispatcher  (GPIO 키 → focused session.send)  │
│   - Rasterizer       (실제 그리기)                      │
...
Rasterizer 는 server 프로세스의 단일 자원. 한 시점에 한 client(=top) 만 명령을 보내므로 race 없음.
...
### Server 프로세스
```
Main thread          AcceptThread          RecvThread (per session)    InputThread
─────────────        ───────────────       ─────────────────────       ────────────
부팅/조립            UdsServer.accept()    UdsChannel.recv()           GpioKeys.wait_event()
launcher fork+exec   → push session        → 역직렬화                  → focus_stack.top()
SIGCHLD reap         → spawn RecvThread    → IRenderCommand            → channel.send(KeyEvent)
                                           → execute(Rasterizer)
                                           EOF → focus_stack.pop()
                                                → 스레드 종료
```
